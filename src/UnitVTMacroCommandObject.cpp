...
bool TVTESC::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0, eventID = VTOnESC;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false;
    int16_t objIdx = -1;
...
bool TVTHideShowObject::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
bool TVTEnableDisableObject::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        ...
        if (objIdx >= 0) {
            ...
            if (pVT_Net->VTObjType == 10) {
                ...
                optn = char(valueAttr[0].toLatin1()) & 0xFE;
                valueAttr = "";
...
bool TVTSelectInputObject::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    QString ss = "", str = "";
    uint8_t *buff;
    bool lCount = getVTObjectListSize(pVT_Net), valid = false, change = false;
    VTError = 0x01;
    if ((lCount > 0) && (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net))) {
            ...
            if (pVT_Net->VTObjType == 5) {
                ...
                if (objIdx >= 0) {
                    ...
                    if (pVT_Net->TFT_KeySelectObjID < 0xFFFF)
                        objID = pVT_Net->TFT_KeySelectObjID;
                    if (pVT_Net->TFT_ButtonSelectObjID < 0xFFFF)
                        objID = pVT_Net->TFT_ButtonSelectObjID;
                    if (pVT_Net->TFT_InputSelectObjID < 0xFFFF)
                        objID = pVT_Net->TFT_InputSelectObjID;
        ...
        if (VTError == 0x00) {
            if (objID < 0xFFFF) {
                ...
                if (objID == pVT_Net->TFT_KeySelectObjID)
                    pVT_Net->TFT_KeySelectObjID = 0xFFFF;
                if (objID == pVT_Net->TFT_ButtonSelectObjID)
                    pVT_Net->TFT_ButtonSelectObjID = 0xFFFF;
                if (objID == pVT_Net->TFT_InputSelectObjID)
                    pVT_Net->TFT_InputSelectObjID = 0xFFFF;
            }
            //
...
bool TVTControlAudioDevice::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false, TEST = false;
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        ...
        //
        if (TEST) {
            qWarning() << pVT_Net->VTRepetition;
            qWarning() << pVT_Net->VTFrequency;
            qWarning() << pVT_Net->VTOnTimeDuration;
            qWarning() << pVT_Net->VTOffTimeDuration;
        }
        //
        for (int i = 2; i < pMsg->LEN; i++)
            pMsg->DATA[i] = 0xFF;
        VTError = pMsg->DATA[1];
        valid = true;
    }
...
bool TVTSetAudioVolume::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false, TEST = false;
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        ...
        pMsg->DATA[1] = 0;
        if (TEST) {
            qWarning() << pVT_Net->VTAudioVolume;
        }
        //
        for (int i = 2; i < pMsg->LEN; i++)
            pMsg->DATA[i] = 0xFF;
...
bool TVTChangeChildLocation::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
bool TVTChangeChildPosition::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    ...
    if ((pStream != NULL) && (pStream->available() > 8))
...
bool TVTChangeSize::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
bool TVTChangeBackgroundColour::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
bool TVTChangeNumericValue::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        VTObjectReference = (pMsg->DATA[1] << 0) + (pMsg->DATA[2] << 8);
        VTValue = (pMsg->DATA[4] << 0) + (pMsg->DATA[5] << 8) + +(pMsg->DATA[6] << 16)
                  + (pMsg->DATA[7] << 24);
        VTError = 0x01;
        //Set new ObjectAttribute
        int16_t objIdx = getVTObjID(pVT_Net, VTObjectReference);
...
bool TVTChangeEndPoint::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
bool TVTChangeFontAttributes::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
bool TVTChangeLineAttributes::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        ...
        if (objIdx >= 0) {
            getVTObjectFromList(pVT_Net, objIdx);
            nameAttr = "VTLineColour";
            valueAttr = "";
            valueAttr += char(pMsg->DATA[3]);
            err += SetVTObjectAttribute(nameAttr, valueAttr, pVT_Net);
            change = ((change) || (pVT_Net->VT_ChangeAttr));
            //
            nameAttr = "VTLineWidth";
            valueAttr = "";
            valueAttr += char(pMsg->DATA[4]);
            err += SetVTObjectAttribute(nameAttr, valueAttr, pVT_Net);
            change = ((change) || (pVT_Net->VT_ChangeAttr));
...
bool TVTChangeFillAttributes::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        ...
        if (objIdx >= 0) {
            ...
            change = ((change) || (pVT_Net->VT_ChangeAttr));
            //
            nameAttr = "VTFillColour";
            valueAttr = "";
            valueAttr += char(pMsg->DATA[4]);
            err += SetVTObjectAttribute(nameAttr, valueAttr, pVT_Net);
            change = ((change) || (pVT_Net->VT_ChangeAttr));
            //
            nameAttr = "VTFillPattern";
            valueAttr = "";
            valueAttr += char(pMsg->DATA[5]);
            valueAttr += char(pMsg->DATA[6]);
            err += SetVTObjectAttribute(nameAttr, valueAttr, pVT_Net);
...
bool TVTChangeActiveMask::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
bool TVTChangeSoftKeyMask::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        ...
        pMsg->DATA[1] = ((VTMaskID >> 0) & 0xFF);
        pMsg->DATA[2] = ((VTMaskID >> 8) & 0xFF);
        pMsg->DATA[3] = ((VTNewMaskID >> 0) & 0xFF);
        pMsg->DATA[4] = ((VTNewMaskID >> 8) & 0xFF);
        VTError = 0x01;
        //Set new ObjectAttribute
        int16_t objIdx = getVTObjID(pVT_Net, VTMaskID);
        if (objIdx >= 0) {
            getVTObjectFromList(pVT_Net, objIdx);
            nameAttr = "VTSoftKeyMask";
            valueAttr = "";
            valueAttr += char(pMsg->DATA[3]);
            valueAttr += char(pMsg->DATA[4]);
            pVT_Net->VT_SetEvent = false;
...
bool TVTChangeAttribute::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        ...
        //TEST
        //qWarning() << VTObjectReference;
        //qWarning() << VTError;
        //qWarning() << QString(VT0PCommFunction,HEX);
        //getMacro events and commands
...
bool TVTChangePriority::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
bool TVTChangeListItem::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0, eventID = VTOnChangeValue;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false;
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        ...
        pMsg->DATA[4] = ((VTNewObjectID >> 0) & 0xFF);
        pMsg->DATA[5] = ((VTNewObjectID >> 8) & 0xFF);
        VTError = 0x01;
        //Set new ObjectAttribute
        int16_t objIdx = getVTObjID(pVT_Net, VTObjectReference);
        if (objIdx >= 0) {
...
bool TVTChangeStringValue::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    uint8_t err = 0, eventID = VTOnChangeValue;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false;
    ...
    uint8_t dst = ((pMsg->ID) >> 8) & 0xFF;
    if ((pStream != NULL) && (pStream->available()))
        ctrlByte = pStream->read();
    //TP_EOMA
    if (((pMsg->ID & 0x00FF0000) == TP_PGN) && (src == pVT_Net->VT_SRC) && (pMsg->DATA[0] == 0x13)
        && (ctrlByte == VT0PCommFunction)) {
        ...
        if (pStream->available())
            VTObjectReference = pStream->readBytesVal(2);
        //Set new ObjectAttribute
        int16_t objIdx = getVTObjID(pVT_Net, VTObjectReference);
        //
        if (objIdx >= 0) {
...
bool TVTChangePolygonPoint::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
bool TVTChangePolygonScale::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false;
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
        ...
        pMsg->DATA[1] = ((VTObjectReference >> 0) & 0xFF);
        pMsg->DATA[2] = ((VTObjectReference >> 8) & 0xFF);
        pMsg->DATA[3] = ((VTNewWidth >> 0) & 0xFF);
        pMsg->DATA[4] = ((VTNewWidth >> 8) & 0xFF);
        pMsg->DATA[5] = ((VTNewHeight >> 0) & 0xFF);
        pMsg->DATA[6] = ((VTNewHeight >> 8) & 0xFF);
        //
        VTError = 0x01;
        //Set new ObjectAttribute
        int16_t objIdx = getVTObjID(pVT_Net, VTObjectReference);
...
//------------------------------------------------------------------------------
//type    //189  $BD
bool TVTLockMaskObject::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false;
...
//------------------------------------------------------------------------------
//type    //190  $BE
bool TVTExecuteMacroObject::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
...
//------------------------------------------------------------------------------
//type    //188 $BC
bool TVTExtendedExecuteMacroObject::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    if (getMsgToAttr(VT0PCommFunction, pMsg, pVT_Net)) {
...
//------------------------------------------------------------------------------
//type    //181  $B5
bool TVTChangeObjectLabel::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false;
...
//------------------------------------------------------------------------------
//type   //184   $B8
bool TVTGraphicContextCommand::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false;
...
//------------------------------------------------------------------------------
//type    //186  $BA
bool TVTSelectColourMapObject::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false;
...
bool TVTExtendedInputAttributeCommand::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    uint8_t err = 0;
    QString nameAttr = "", valueAttr = "";
    bool valid = false, change = false;
...
//------------------------------------------------------------------------------
//type  //144  $90
bool TVTSelectActiveWorkingSet::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    ...
    if (((pMsg->ID & 0x00FF0000) == TP_PGN) && (src == pVT_Net->VT_SRC) && (pMsg->DATA[0] == 0x13)
        ...
        for (i = 0; i < 8; i++) {
            if (pStream->available())
                VTWorkingSetName = getStringHEX(pStream->read(), 2) + VTWorkingSetName;
        } //for i
        //reset selected Input
        resetSelectInput(pVT_Net);
        //TEST
        qWarning() << "VTWorkingSetName";
        qWarning() << VTWorkingSetName;
        qWarning() << pVT_Net->VT_DST_WS[0];
        qWarning() << pVT_Net->VT_DST_WS[1];
        //
        for (i = 0; i < listMax; i++) {
