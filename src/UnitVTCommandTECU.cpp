...
bool TVTAcknowledgeMessage::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
...
QString getSpeedMessage(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    if ((pMsg->ID & 0x00FFFF00) == TECU_WHEEL_PGN) {
        ...
        pVT_Net->tecuData.Distance = Distance;
        k = 1;
    }
    pVT_Net->tecuData.objValue1[k] = bSpeed;
    QString str = QString::number(round(3.6 * bSpeed / 1000)) + " km/h";
    return str;
}; //getSpeedMessage
...
QString getEngineSpeedMessage(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    QString str = QString::number(round(eSpeed / 8)) + " rpm";
    return str;
...
QString getHitchStatusMessage(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    QString str = QString::number(round(hitch)) + " %";
    return str;
...
QString getPTOOutShaftMessage(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    QString str = QString::number(round(pto)) + " rpm";
    return str;
...
bool getTECUMonitor(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    ...
    if (pVT_Net->listNr == (listMax - 1)) {
        ...
        if ((pMsg->ID & 0x00FFFF00) == TECU_WHEEL_PGN) {
            k = 0;
            str = getSpeedMessage(pMsg, pVT_Net);
            valid = (str.length() > 0);
        }
        //Ground=0x00FE4900
        ...
        if ((pMsg->ID & 0x00FFFF00) == TECU_RearPTOOutShaft_PGN) {
            k = 5;
            str = getPTOOutShaftMessage(pMsg, pVT_Net);
            valid = (str.length() > 0);
        }
