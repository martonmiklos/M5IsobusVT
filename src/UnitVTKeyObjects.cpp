...
//==============================================================================
//==============================================================================
bool TVTSoftKeyMask::PaintObjTo(TVT_ViewRect *pViewRect, TVT_Net *pVT_Net)
{
...
//------------------------------------------------------------------------------
bool TVTSoftKeyMask::writeToStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    return writeToStreamDirect(pVT_Net, pStream);
};

//------------------------------------------------------------------------------
bool TVTSoftKeyMask::readFromStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
...
void TVTSoftKeyMask::setAID()
{
    ...
    VTAttrAID[2].nameAID = "VTItemList";
    VTAttrAID[2].valueAID = VTItems;
...
//==============================================================================
//==============================================================================
bool TVTKey::PaintObjTo(TVT_ViewRect *pViewRect, TVT_Net *pVT_Net)
{
    bool valid = (getVTObjectListSize(pVT_Net) > 0);
    ...
    if ((valid) && (objIdx >= 0)) {
        getAID();
        pVT_Net->level++;
        if (pVT_Net->VTKeyIndex == 255)
            pVT_Net->VTKeyIndex = 0;
        //Paint
        ...
        pVT_Net->SetLabel--;
        getViewport(pVT_Net, &vRect, x, y, w, h);
        Set_drawRect(pVT_Net, x, y, w, h, 0 /*TFT_BLUE*/); // FIXME
        //
...
//------------------------------------------------------------------------------
bool TVTKey::writeToStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    return writeToStreamDirect(pVT_Net, pStream);
};

//------------------------------------------------------------------------------
bool TVTKey::readFromStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    bool valid = (pStream->available());
    uint16_t i = 0, j = 0, nn = 0;
    uint8_t bb = 0;
...
void TVTKey::setAID()
{
    ...
    VTAttrAID[2].nameAID = "VTKeyCode";
    VTAttrAID[2].valueAID = QString::number(VTKeyCode);
    //
    VTAttrAID[3].numAID = 3;
    VTAttrAID[3].byteAID = 1;
    VTAttrAID[3].typeAID = 2;
...
//==============================================================================
//==============================================================================
bool TVTButton::PaintObjTo(TVT_ViewRect *pViewRect, TVT_Net *pVT_Net)
{
    bool valid = (getVTObjectListSize(pVT_Net) > 0);
    int16_t x = pVT_Net->x, y = pVT_Net->y;
    ...
    if (pViewRect == NULL) {
        ...
        if (pVT_Net->nameAttr != "") {
            ...
            if (pVT_Net->nameAttr == "VTEnabled") {
                ...
                bitWrite8(VTOptions, 4, !char(pVT_Net->newValueAttr[0].toLatin1())); //set bit 4
    ...
    if ((valid) && (objIdx >= 0)) {
        getAID();
        pVT_Net->level++;
        if (pVT_Net->VTButtonIndex == 255)
            pVT_Net->VTButtonIndex = 0;
        //Paint
        ...
        if ((VTObjID == pVT_Net->TFT_ButtonSelectObjID) && (pVT_Net->TFT_ButtonSelect < 255)) {
            ...
            if ((VTOptions & 0x01) == 0x01) {
                if (pVT_Net->TFT_ButtonPressed == 1)
                    VTOptions = (VTOptions | 0x02);
                if (pVT_Net->TFT_ButtonPressed == 0)
                    VTOptions = (VTOptions & 0xFD);
                //TEST
                //qWarning() << "VTOptions=" + QQString::number(VTOptions);
                if (pVT_Net->TFT_ButtonPressed >= 0) {
                    pVT_Net->nameAttr = "VTOptions";
                    pVT_Net->newValueAttr = char(VTOptions);
        ...
        //
        if (tr == 0)
            Set_fillRect(pVT_Net, xx, yy, ww, hh, VTBackgroundColour);
        Set_drawRect(pVT_Net, xx, yy, ww, hh, 0);
        if ((nb == 1) && (la == 1) && (ls == 1))
            Set_drawRect(pVT_Net, xx + 1, yy + 1, ww - 2, hh - 2, 0);
        //
        getViewport(pVT_Net, &vRect, x, y, w, h);
...
//------------------------------------------------------------------------------
bool TVTButton::writeToStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    return writeToStreamDirect(pVT_Net, pStream);
};

//------------------------------------------------------------------------------
bool TVTButton::readFromStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    bool valid = (pStream->available());
    uint16_t i = 0, j = 0, nn = 0;
    uint8_t bb = 0;
...
void TVTButton::setAID()
{
    ...
    VTAttrAID[4].nameAID = "VTBorderColour";
    VTAttrAID[4].valueAID = QString::number(VTBorderColour);
    VTAttrAID[5].numAID = 5;
    VTAttrAID[5].byteAID = 1;
    VTAttrAID[5].typeAID = 1;
    VTAttrAID[5].nameAID = "VTKeyCode";
    VTAttrAID[5].valueAID = QString::number(VTKeyCode);
    ...
    VTAttrAID[7].nameAID = "VTObjects";
    VTAttrAID[7].valueAID = VTObjects;
    VTAttrAID[8].numAID = 8;
    VTAttrAID[8].byteAID = 1;
    VTAttrAID[8].typeAID = 2;
    VTAttrAID[8].nameAID = "VTMacros";
    VTAttrAID[8].valueAID = VTMacros;
    VT_AID_Nr = 9;
};
...
//VTObjType=35
//==============================================================================
bool TVTKeyGroupObject::PaintObjTo(TVT_ViewRect *pViewRect, TVT_Net *pVT_Net)
{
    bool valid = (getVTObjectListSize(pVT_Net) > 0);
    uint8_t cIdx = 0, err = 0x00;
    uint16_t colTransp = /*TFT_WHITE*/ 0 /* FIXME */, ww0 = 30, vW, vH, ww, hh, w, h, oLevel = pVT_Net->level;
...
//------------------------------------------------------------------------------
bool TVTKeyGroupObject::writeToStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    return writeToStreamDirect(pVT_Net, pStream);
};

//------------------------------------------------------------------------------
bool TVTKeyGroupObject::readFromStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    bool valid = (pStream->available());
    uint16_t i = 0, j = 0, nn = 0;
    uint8_t bb = 0;
...
void TVTKeyGroupObject::setAID()
{
    ...
    VTAttrAID[1].byteAID = 1;
    VTAttrAID[1].typeAID = 1;
    VTAttrAID[1].nameAID = "VTOptions";
    VTAttrAID[1].valueAID = QString::number(VTOptions);
    VTAttrAID[2].numAID = 2;
    VTAttrAID[2].byteAID = 2;
    VTAttrAID[2].typeAID = 1;
    VTAttrAID[2].nameAID = "VTName";
    VTAttrAID[2].valueAID = QString::number(VTName);
    VTAttrAID[3].numAID = 3;
    VTAttrAID[3].byteAID = 2;
    VTAttrAID[3].typeAID = 2;
    VTAttrAID[3].nameAID = "VTKeyGroupIcon";
    VTAttrAID[3].valueAID = QString::number(VTKeyGroupIcon);
    //
    VTAttrAID[4].numAID = 4;
    VTAttrAID[4].byteAID = 1;
    VTAttrAID[4].typeAID = 2;
    VTAttrAID[4].nameAID = "VTItemList";
    VTAttrAID[4].valueAID = VTItems;
    VTAttrAID[5].numAID = 5;
    VTAttrAID[5].byteAID = 1;
...
//VTObjType=40
//==============================================================================
bool TVTLabelReferenceListObject::PaintObjTo(TVT_ViewRect *pViewRect, TVT_Net *pVT_Net)
{
    bool valid = (getVTObjectListSize(pVT_Net) > 0);
    uint8_t cIdx = 0, err = 0x00;
...
//------------------------------------------------------------------------------
bool TVTLabelReferenceListObject::writeToStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    return writeToStreamDirect(pVT_Net, pStream);
};

//------------------------------------------------------------------------------
bool TVTLabelReferenceListObject::readFromStream(TVT_Net *pVT_Net, LoopbackStream *pStream)
{
    bool valid = (pStream->available());
    uint16_t i = 0, j = 0, nn = 0;
    uint8_t bb = 0;
    ...
    if (valid) {
        ...
        for (i = 1; i < VT_AID_Nr; i++) {
            nn = VTAttrAID[i].valueAID.toInt();
            if (i == 1)
                getVTLabels(pVT_Net, nn, pStream);
        } //for i
        //
        getVTObjectListAdd(pVT_Net);
        pVT_Net->streamStr.clear();
...
void TVTLabelReferenceListObject::setAID()
{
    ...
    VTAttrAID[1].nameAID = "VTLabels";
    VTAttrAID[1].valueAID = VTLabels;
    VT_AID_Nr = 2;
}
