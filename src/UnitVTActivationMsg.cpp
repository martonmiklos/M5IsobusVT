...
bool TVTMaintenanceMessage::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
...
bool TVT_VTStatusMessage::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
...
bool TVT_VTSoftKeyActivation::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    uint16_t objID = pVT_Net->VTObjID;
    uint8_t keyNr = pVT_Net->TFT_KeySelect + 1, eventID = VTOnSoftKeyPress; //24;
    pMsg->ID = VTtoECU_PGN + (ECU_VT_PRIO << 24) + (pVT_Net->VT_DST[pVT_Net->listNr] << 8)
               + pVT_Net->VT_SRC;
    pMsg->MSG_TX = 1;
    pMsg->MSGTYPE = 1;
    pMsg->LEN = 8;
    if (pVT_Net->VTObjID == 0xFFFF)
        keyNr = 0; //ACKN
    //
    pMsg->DATA[0] = VT0PCommFunction;
...
bool TVT_VTButtonActivation::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    uint16_t objID = pVT_Net->VTObjID;
    uint8_t keyNr = pVT_Net->TFT_KeySelect + 1, eventID = VTOnSoftKeyPress; //24;
    ...
    if (objIdx >= 0) {
        ...
        if (!ds) {
            ...
            if (valid) {
                ...
                //qWarning() << "getVTMacrosListEvents=" + QString(pVT_Net->VTObjID) + "/" + QString(eventID);
                getVTMacrosListEvents(pVT_Net, pVT_Net->VTObjID, eventID);
            }
        } //not ds
    } //objIdx>=0
...
bool TVT_VTPointingEvent::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    uint16_t objID = pVT_Net->VTObjID;
    ...
    if ((pMsg->DATA[5] & 0x0F) == 0)
        eventID = VTOnPointingRelease; //28;
    getVTMacrosListEvents(pVT_Net, pVT_Net->VTObjID, eventID);
...
bool TVT_VTSelectInput::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    uint16_t objID = pVT_Net->VTObjID;
    ...
    pMsg->DATA[7] = 0x0F; //TAN number
    if (keyNr == 255)
        eventID = VTOnInputFieldDeselection; //20
                                             //
    getVTMacrosListEvents(pVT_Net, pVT_Net->VTObjID, eventID);
    pVT_Net->VTPoolRefresh = true;
    valid = true;
...
bool TVT_VTESC::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    uint16_t objID = pVT_Net->VTObjID;
...
bool TVT_VTChangeNumericValue::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false, TEST = false;
    uint8_t keyNr = 0xFF, nn = 0, mm = 0, dz = 0, frmt = 0, just = 0, en = 0,
            eventID = VTOnEntryOfValue; //=22   //VTOnEntryOfNewValue;//23
    int16_t refIdx = -1;
    ...
    if (HasInArray(pVT_Net->VTObjType, inpNumObjSet)) {
        ...
        if (refIdx >= 0) {
            ...
            if (pVT_Net->VTObjType == 7) {
                nn = 8;
                refObj = pVT_Net->streamStr.readBytesVal(2, nn);
                nn += 2;
                VTValue = pVT_Net->streamStr.readBytesVal(1, nn);
                if (refObj < 0xFFFF) {
                    refIdx = getVTObjID(pVT_Net, refObj);
                    refObj = 0xFFFF;
                    if (refIdx >= 0) {
                ...
                if (pVT_Net->VTValueStr.length() == 0) {
                    ...
                    pVT_Net->VTValue = pVT_Net->VTValueStr.toInt();
                //
            ...
            if (pVT_Net->VTObjType == 10) {
                ...
                if (pVT_Net->VTValueStr.length() == 0) {
                    pVT_Net->VTValue = VTValue;
                    if ((pVT_Net->VTValue >= 255) || (pVT_Net->VTValue >= vMax))
                        pVT_Net->VTValue = 0;
                } else {
                    ...
                    if (pVT_Net->VTValue > 0xFF)
                        pVT_Net->VTValue = 0xFF;
                }
                //
                if (pVT_Net->getVTValue) {
                    pVT_Net->VTValue = VTValue;
                    pVT_Net->VTValueStr = QString::number(VTValue);
                    return true;
                } else {
                    if (VTValue != pVT_Net->VTValue) {
                        VTValue = pVT_Net->VTValue;
            ...
            if (pVT_Net->VTObjType == 9) {
                ...
                nn += 2;
                //actual value
                VTValue = pVT_Net->streamStr.readBytesVal(4, nn);
                nn += 4;
                //minValue
                vMin = pVT_Net->streamStr.readBytesVal(4, nn);
                nn += 4;
                //maxValue
                vMax = pVT_Net->streamStr.readBytesVal(4, nn);
                nn += 4;
                //offs
                offs = pVT_Net->streamStr.readBytesVal(4, nn);
                nn += 4;
                //Scale
                int4 = pVT_Net->streamStr.readBytesVal(4, nn);
                nn += 4;
                rr = getFloatFromInt(int4);
                //Decimals
                dz = pVT_Net->streamStr.readBytesVal(1, nn);
                nn += 1;
                //Format
                frmt = pVT_Net->streamStr.readBytesVal(1, nn);
                nn += 1;
                //Justification
                just = pVT_Net->streamStr.readBytesVal(1, nn);
                nn += 1;
                //Enabled
                en = pVT_Net->streamStr.readBytesVal(1, nn);
                nn += 1;
                //
                if (refObj < 0xFFFF) {
                    refIdx = getVTObjID(pVT_Net, refObj);
            ...
            if (TEST)
                getStreamStrInfo(pVT_Net);
            if ((valid) && (eventID == VTOnEntryOfNewValue))
        ...
        if (valid) {
            ...
            if (eventID == VTOnEntryOfNewValue) {
                //qWarning() << "VTOnEntryOfNewValue=" + QString(eventID);
...
bool TVT_VTChangeActiveMask::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    pMsg->ID = VTtoECU_PGN + (ECU_VT_PRIO << 24) + (pVT_Net->VT_DST[pVT_Net->listNr] << 8)
               + pVT_Net->VT_SRC;
...
bool TVT_VTChangeSoftKeyMask::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    pMsg->ID = VTtoECU_PGN + (ECU_VT_PRIO << 24) + (pVT_Net->VT_DST[pVT_Net->listNr] << 8)
               + pVT_Net->VT_SRC;
...
bool TVT_VTInputStringValue::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    uint8_t olen = 0, nlen = 0, nn = 0, i = 0,
            eventID = VTOnEntryOfValue; //=22   //VTOnEntryOfNewValue;//23
    ...
    if (pVT_Net->VTObjType == gInpStringType) {
        ...
        if ((refIdx >= 0) && (pVT_Net->VTObjType == 8)) {
            nn = 8;
            pVT_Net->VTInpFont = pVT_Net->streamStr.readBytesVal(2, nn);
            nn += 2;
            pVT_Net->VTInpAttr = pVT_Net->streamStr.readBytesVal(2, nn);
            nn += 3;
            refObj = pVT_Net->streamStr.readBytesVal(2, nn);
            nn += 3;
            olen = pVT_Net->streamStr.readBytesVal(1, nn);
            nn++;
            //actual value
            VTNewValue = "";
            for (i = 0; i < olen; i++)
                VTNewValue += char(0x20);
            pVT_Net->streamStr.readBytes((uint8_t *) VTNewValue.constData(), olen, nn);
            //
            if (refObj < 0xFFFF) {
                refIdx = getVTObjID(pVT_Net, refObj);
                refObj = 0xFFFF;
                if (refIdx >= 0) {
                    refObj = pVT_Net->VTObjID;
                    nn = 3;
                    olen = pVT_Net->streamStr.readBytesVal(2, nn);
                    nn += 2;
                    //actual value
                    VTNewValue = "";
                    for (i = 0; i < olen; i++)
            ...
            if (pVT_Net->VTValueStr.length() == 0) {
                ...
                if (nlen > olen)
                    pVT_Net->VTValueStr = pVT_Net->VTValueStr.mid(0, olen);
                //
                if (VTNewValue != pVT_Net->VTValueStr) {
                    while (pVT_Net->VTValueStr.length() < olen)
                        pVT_Net->VTValueStr += char(0x20);
                    VTNewValue = pVT_Net->VTValueStr;
                    nn = 17;
                    eventID = VTOnEntryOfNewValue; //23
                    if (refObj < 0xFFFF)
                        nn = 5;
                    pVT_Net->streamStr.writeBytes((uint8_t *) VTNewValue.constData(), olen, nn);
                    setVTObjectListValue(pVT_Net, refIdx);
                }
                VTNewValue = VTNewValue.trimmed();
        ...
        if (valid) {
            ...
            if (nlen > 4) {
                ...
                pVT_Net->stream_TP[pVT_Net->listNr].writeBytes((uint8_t *) VTNewValue.constData(), nlen);
                //
                //TX3 1CEC0A26  8  10 09 00 02 FF 00 E6 00
                //[TP.CM_RTS_E600_00_0]  Size=nn9  Packets=2
                //VTtoECU_PGN
                pMsg->ID = (TP_ETP_PRIO << 24) + TP_PGN + (pVT_Net->VT_DST[pVT_Net->listNr] << 8)
                           + pVT_Net->VT_SRC;
                ...
                //set QString or TP
                for (i = 0; i < nlen; i++)
                    pMsg->DATA[4 + i] = VTNewValue[i].toLatin1();
            }
            //
            refObj = pVT_Net->VTObjID;
...
bool TVT_VTDisplayActivation::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    pMsg->ID = VTtoECU_PGN + (ECU_VT_PRIO << 24) + (pVT_Net->VT_DST[pVT_Net->listNr] << 8)
               + pVT_Net->VT_SRC;
...
bool TVT_VTControlAudioDeviceTermination::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    pMsg->ID = VTtoECU_PGN + (ECU_VT_PRIO << 24) + (pVT_Net->VT_DST[pVT_Net->listNr] << 8)
               + pVT_Net->VT_SRC;
    ...
    pVT_Net->VTActiveTime = QDateTime::currentDateTime().toMSecsSinceEpoch();
...
bool TVTUnsupportedFunction::setMsgToAttr(CANMsg *pMsg, TVT_Net *pVT_Net)
{
    bool valid = false;
    pMsg->ID = VTtoECU_PGN + (ECU_VT_PRIO << 24) + (pVT_Net->VT_DST[pVT_Net->listNr] << 8)
               + pVT_Net->VT_SRC;
